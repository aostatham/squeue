...
public class ApiExceptionHandler {
    ...
    ResponseEntity<LinkedHashMap<String, Object>> queueException(QueueException ex) {
        ...
        return ResponseEntity.status(status).body(errorBody(ex.errorCode().name(), responseMessage(ex), ex.queueName(), ex.sourceId(), ex.itemId() == null ? null : ex.itemId().toString()));
    }
